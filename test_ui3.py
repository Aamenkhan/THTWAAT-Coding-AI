...
def run_test():
    ...
    # Make combo box editable so we can set a custom value
    window.toolbar.model_selector.setEditable(True)
    window.toolbar.model_selector.setCurrentText("llama3")
    ...
    def check():
        ...
        print('-------------------')
        if '[Error:' in out:
            print("ERROR IN UI CONFIRMED")
            app.quit()
        elif 'Agent:' in out and len(out.split('Agent:')[1]) > 10:
